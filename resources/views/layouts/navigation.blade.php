<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <x-nav-link :href="Auth::user()->usertype == 'admin' ? route('admin.dashboard') : route('dashboard')" :active="Auth::user()->usertype == 'admin' ? request()->routeIs('admin.dashboard') : request()->routeIs('dashboard')">
                    ...
                    <x-nav-link href="{{ url('admin/products') }}" :active="request()->routeIs('admin.products')">
                        {{ __('Product') }}
                        </x-nav-link>

                    <x-nav-link href="{{ url('admin/pelanggans') }}" :active="request()->routeIs('admin.pelanggans')">
                        {{ __('User') }}
                        </x-nav-link>

                    {{-- <x-nav-link href="{{ url('admin/user') }}" :active="request()->routeIs('admin.user')">
                        {{ __('User') }}
                        </x-nav-link> --}}
                    @endif
                    ...
                    <x-nav-link href="favorit" :active="request()->routeIs('user.favorit')">
                        {{ __('Favorit') }}
                        </x-nav-link>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            {{-- <x-responsive-nav-link href="user" :active="request()->routeIs('profileadmin.user')">
                {{ __('User') }}
                </x-responsive-nav-link> --}}
            @endif
            ...
            <x-responsive-nav-link href="favorit" :active="request()->routeIs('user.favorit')">
                {{ __('Favorit') }}
                </x-responsive-nav-link>
